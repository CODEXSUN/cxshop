...
## Enforce the standalone event-driven runtime

- Use the fixed MariaDB database selected by the server. Do not add tenant-database routing or client-selected database headers.
- Keep commands, domain rules, migrations, repositories, services, routes, seeds, events, outboxes, relays, workers, and tests in the owning leaf module.
- Persist state and its outbox event in one MariaDB transaction.
- Treat the MariaDB queue and outbox records as durable truth. Redis or BullMQ may only accelerate delivery or caching.
- Register module workers through a public composition contract. Never add business cases to a central job-name switch.
- Implement deterministic matching first in a pure domain function. Persist its evidence. Enqueue semantic matching only after deterministic no-match.
- Require unit tests for matching priority, MariaDB persistence tests for the state-plus-outbox write, retry tests for relay and worker failures, and a composed E2E test for the public flow.
