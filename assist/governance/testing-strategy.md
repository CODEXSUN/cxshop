...
Backend event-driven flows may use executable TypeScript E2E programs when they must prove real MariaDB transactions, outbox rows, queue acceptance, retries, and worker results without a browser.
...
- Event publication.
- Queue job behavior.
- Atomic state-plus-outbox persistence in MariaDB.
- Idempotent relay and worker replay.
- Operation with Redis disabled.
- Deterministic matching priority before semantic fallback.
