...
- CXShop is one standalone product runtime. New modules use the server-selected `cxshop_db`; clients must not choose a database or supply tenant-database routing headers.
- Each asynchronous business module owns its event contracts, transactional outbox, relay, handler, retry rules, persistence tests, and E2E scenario.
- A state change and its event must commit in one MariaDB transaction. Publishing directly before commit or writing the outbox in a later transaction is prohibited.
- MariaDB outbox and queue rows are the durable source of truth. Redis and BullMQ are optional delivery or cache layers and must not be required to recover accepted work.
- Platform queue infrastructure may store, lease, retry, and dispatch jobs. Business handlers remain module-owned and register through public composition contracts; central business job switches are prohibited for new work.
- Matching runs deterministic rules first and persists the rule and confidence. Semantic matching is an asynchronous fallback only after deterministic no-match.
...
- Queue-backed work starts on the MariaDB queue backend in every environment and keeps the same job contract when optional BullMQ + Redis delivery is enabled. Job payloads must include correlation ID, source module, retry policy, idempotency key, and masked operator-visible payload/result details.
...
- Use the shared workspace list, table, pagination, show, upsert, banner, autocomplete, select, date, tab, toast, and status components from `@cxshop/ui`.
- Use the shadcn/Radix themed design-system select (`WorkspaceSelect` or `Select` from `@cxshop/ui`) for all form selects; do not use raw native `<select>` in workspace/list/upsert screens.
