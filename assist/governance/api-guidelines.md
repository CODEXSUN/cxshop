...
Frontend, desktop, mobile, CLI, and internal tools should use `@cxshop/api-client` where practical.
...
| --------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------- |
| `requireSession(app, request)`                | Resolves active session from Bearer token or `cxshop_session` cookie | `AppError.unauthorized()` if missing or invalid               |
...
| ----------------------------- | ---------------------------------------- | ------------------------ |
| Auth (login, session, logout) | `apps/platform/api/src/auth/routes.ts`   | `@cxshop/platform-api` |
| Tenant management CRUD        | `apps/platform/api/src/tenant/routes.ts` | `@cxshop/platform-api` |

### Audit Events

Auth and tenant mutations write audit events through `AuditService` (`@cxshop/platform/audit`).
