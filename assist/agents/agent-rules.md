...
Legitimate infrastructure must be distinguished from business logic. `shared/api` transport/session context, environment readers, observability, and reusable controls from `@cxshop/ui` may be shared only when they contain no module-specific fields, validation, persistence, lifecycle, routes, or workflows. Moving business logic into `shared`, `common`, `foundation`, `helpers`, or `utils` does not make it infrastructure.
