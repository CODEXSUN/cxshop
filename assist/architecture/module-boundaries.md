...
the public `@cxshop/devkit-api` and `@cxshop/devkit-web` contracts. DevKit request databases and actors must always
...
Allowed shared infrastructure is narrow: API transport/session context, environment readers, observability, and reusable `@cxshop/ui` controls. Shared code must not know a business module's fields, validation, tables, lifecycle, routes, forms, lists, workspaces, settings, or print behavior.
