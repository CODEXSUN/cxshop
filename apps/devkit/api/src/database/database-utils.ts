...
export async function renameLegacyTable<Database>(
  ...
  if (await tableExists(database, ownedName)) {
    throw new Error(
      `Cannot rename legacy DevKit table ${legacyName}: ${ownedName} already exists.`
    );
  }
  await sql
    .raw(`RENAME TABLE ${quoteIdentifier(legacyName)} TO ${quoteIdentifier(ownedName)}`)
    .execute(database);
}

async function tableExists<Database>(database: Kysely<Database>, tableName: string) {
