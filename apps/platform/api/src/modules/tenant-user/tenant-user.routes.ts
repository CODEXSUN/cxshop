...
export async function registerTenantUserRoutes(app: FastifyInstance) {
  ...
  registerStatusAction(app, "activate", "active");
  registerStatusAction(app, "deactivate", "inactive");
  registerStatusAction(app, "suspend", "suspended");
  registerContractRoute(app, {
    method: "DELETE",
    url: `${path}/:id/force`,
    schemas: { params, response: record },
...
function registerStatusAction(app: FastifyInstance, name: string, value: z.infer<typeof status>) {
  registerContractRoute(app, {
    method: "POST",
    url: `${path}/:id/${name}`,
    schemas: { params, response: record },
