...
function liveRestoreName(sourceDatabaseName: string, confirmation: string | undefined) {
  if (
    env.CXSHOP_ALLOW_LIVE_RESTORE !== "1" ||
    env.CXSHOP_LIVE_RESTORE_CONFIRM !== "ALLOW_LIVE_RESTORE"
  ) {
    throw new Error(
      "Live restore is disabled. Set CXSHOP_ALLOW_LIVE_RESTORE=1 and CXSHOP_LIVE_RESTORE_CONFIRM=ALLOW_LIVE_RESTORE only during an approved restore window."
    );
  }
...
function restoreSandboxName(sourceDatabaseName: string) {
  if (env.NODE_ENV === "production" && !env.CXSHOP_RESTORE_TEST_DB_NAME) {
    throw new Error("Production restore requires CXSHOP_RESTORE_TEST_DB_NAME.");
  }
  return assertDatabaseName(
    env.CXSHOP_RESTORE_TEST_DB_NAME || `${sourceDatabaseName}_restore_sandbox`,
