...
} from "../modules/cloud-publishing/blog-cloud-database.js";
...
async function rollbackAll() {
  if (
    ...
    process.env.CXSHOP_MIGRATION_ROLLBACK_CONFIRM !== "ROLLBACK"
  ) {
    throw new Error(
      "production rollback refused. Set CXSHOP_MIGRATION_ROLLBACK_CONFIRM=ROLLBACK after verifying the backup and rollback plan."
    );
  }
...
async function runMigrationPreflight() {
  ...
  if (env.NODE_ENV === "production" && !process.env.CXSHOP_VERIFIED_BACKUP_ID) {
    throw new Error(
      "production migration preflight refused. Set CXSHOP_VERIFIED_BACKUP_ID to the verified pre-migration backup id."
    );
  }
...
async function runLocalMigrationTest() {
  ...
  if (process.env.CXSHOP_RESTORED_DUMP_TEST !== "1") {
    throw new Error(
      "local migration test requires CXSHOP_RESTORED_DUMP_TEST=1 after restoring a dump into the configured local databases."
    );
  }
...
async function runRestoreTest() {
  ...
  const restoreDatabaseName = process.env.CXSHOP_RESTORE_TEST_DB_NAME;
  if (!restoreDatabaseName) {
    throw new Error(
      "restore test requires CXSHOP_RESTORE_TEST_DB_NAME set to an explicit sandbox database name."
    );
  }
...
async function runBackupVerify() {
  const backupId = process.env.CXSHOP_BACKUP_VERIFY_ID;
  if (!backupId) {
    throw new Error(
      "backup verification requires CXSHOP_BACKUP_VERIFY_ID set to the backup artifact/run id being verified."
    );
  }
  console.info(
...
This deletes configured database data. Required guards:
  CXSHOP_DB_RESET_CONFIRM=DROP_DATABASES
  CXSHOP_ALLOW_PRODUCTION_DB_RESET=1 when NODE_ENV=production
`);
}
