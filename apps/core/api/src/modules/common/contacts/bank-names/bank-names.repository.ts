...
export class BankNamesRepository {
  async list(filters: BankNamesListFilters = {}) {
    const rows = await sql<BankNamesRow>`SELECT id, name, status, sort_order FROM core_bank_names
  ...
  async create(input: BankNamesSavePayload) {
    const result = await sql`INSERT INTO core_bank_names (name, status, sort_order) VALUES
  ...
  async update(id: string | number, input: BankNamesSavePayload) {
    ...
    await sql`UPDATE core_bank_names SET name=${normalizeString(input.name)}, status=${input.isActive === false ? "inactive" : "active"},
  ...
  async setActive(id: string | number, isActive: boolean) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`UPDATE core_bank_names SET status=${isActive ? "active" : "inactive"}, updated_at=CURRENT_TIMESTAMP WHERE id=${Number(id)}`.execute(
      getCoreDatabase()
    );
    return this.find(id);
  }
  ...
  async forceDelete(id: string | number) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`DELETE FROM core_bank_names WHERE id=${Number(id)}`.execute(getCoreDatabase());
    return existing;
  }
}
