...
import { uploadPromotionImage } from "./promotion-card.services";

const maxBytes = 8 * 1024 * 1024;

export function PromotionCardImageUpload({
  code,
  onChange,
  onError,
  value
  ...
  const upload = async (file: File) => {
    ...
    if (file.size > maxBytes) return onError("Promotion image must be smaller than 8 MB.");
    setUploading(true);
    onError("");
    try {
      const extension = file.name.split(".").at(-1)?.toLowerCase() || "webp";
      const uploaded = await uploadPromotionImage(
        `${slugify(code || "promotion")}-${Date.now()}.${extension}`,
        await fileBase64(file)
      );
      onChange(uploaded.imageUrl);
    } catch (caught) {
      onError(caught instanceof Error ? caught.message : "Could not upload the promotion image.");
    } finally {
      setUploading(false);
