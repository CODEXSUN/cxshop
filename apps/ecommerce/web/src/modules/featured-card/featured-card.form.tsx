...
import { useFeaturedCardFrappeItems } from "./featured-card.hooks";
import { featuredCardSchema } from "./featured-card.schema";
import { FeaturedCardImageUpload } from "./featured-card.image-upload";
import { getFrappeFeaturedItem } from "./featured-card.services";
import type { FeaturedCardPayload, FeaturedCardRecord } from "./featured-card.types";

const empty: FeaturedCardPayload = {
...
export function FeaturedCardForm({
  ...
  loading: boolean;
  onCancel: () => void;
  onSubmit: (value: FeaturedCardPayload) => void;
  record: FeaturedCardRecord | null;
}) {
  ...
  const frappeItems = useFeaturedCardFrappeItems(frappeSearch);
  ...
  const update = <K extends keyof FeaturedCardPayload>(key: K, next: FeaturedCardPayload[K]) =>
    setValue((current) => ({ ...current, [key]: next }));
  const submit = () => {
    const parsed = featuredCardSchema.safeParse(value);
    if (!parsed.success) return setError(parsed.error.issues[0]?.message ?? "Check the featured.");
    setError("");
  ...
  const selectFrappeItem = async (itemCode: string) => {
    ...
    try {
      const item = await getFrappeFeaturedItem(itemCode);
      setValue((current) => ({
        ...current,
        ...
        featuredCode: slugify(item.itemCode),
        title: item.itemName
      }));
      setFrappeMessage(
        `Filled from Frappe item ${item.itemCode}. Review the featured fields before saving.`
      );
    } catch (caught) {
      setFrappeMessage(
  ...
  return (
    <Card
      title={record ? "Edit featured card" : "New featured card"}
      description="Manage the local storefront copy of a featured card. Pull from Frappe first when linking an iShop item."
    >
      {frappeMessage ? (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Featured code *">
          <Input
            value={value.featuredCode}
            onChange={(event) => update("featuredCode", event.target.value)}
          />
        </Field>
        <Field label="Title *">
        ...
        <Field label="Featured image">
          <FeaturedCardImageUpload
            code={value.featuredCode}
        ...
        <Field label="Badge position">
          <select
            ...
            onChange={(event) =>
              update("badgePosition", event.target.value as FeaturedCardPayload["badgePosition"])
            }
          >
      ...
      {value.imageUrl ? (
        <img
          alt={value.title || "Featured preview"}
...
function payload(record: FeaturedCardRecord): FeaturedCardPayload {
  const {
    createdAt: _created,
