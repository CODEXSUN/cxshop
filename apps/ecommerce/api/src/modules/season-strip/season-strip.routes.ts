...
import { SeasonStripService } from "./season-strip.service.js";

const base = "/ecommerce/storefront/season-strips";
...
const payload = z.object({
  ...
  seasonCode: z.string().trim().min(1).max(191),
...
export async function registerSeasonStripRoutes(app: FastifyInstance) {
  const service = new SeasonStripService();
...
function required<T>(value: T | null) {
  if (!value) throw AppError.notFound("Season strip was not found.");
  return value;
}
